...
class BoWClassifier(nn.Module):
    def __init__(self, num_labels, embedding_dim, vocab_size, weight):
...
def main():
    ...
    model_2 = BoWClassifier(num_labels, Embedding_dim, size, weight)
    ...
    for epoch in range(5):
        ...
        for t in range(len(label)):
            ...
            if t % 1000 == 0:
                print(t)
        print(epoch)
    torch.save(model_2.state_dict(), "/Users/xietiany/Documents/NLP/models/NN_glove_Embedding_model")
    ...
    # with torch.no_grad():
    #     outfile = open("P3_unlabelled.output", "w")
    #     for i in range(len(label_test)):
    #         first_input = create_bag_word(text_test[i], size, bag_of_word_dic)
    #         context_idxs = torch.tensor(first_input, dtype=torch.long)
